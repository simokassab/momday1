	...
	public function view($route, $data = array()) {
		...
		if ($result && !$result instanceof Exception) {
			...
			foreach ($data as $key => $value) {
              ...
              if ($this->config->get('marketplace_separate_view') && isset($this->session->data['marketplace_separate_view']) && $this->session->data['marketplace_separate_view'] == 'separate' && preg_match('/account\/customerpartner/',$route) && isset(explode('/',$route,2)[1])) {
                $output = $template->render('default/template/' . $route, $this->registry->get('config')->get('template_cache'));
              } elseif ($this->config->get('marketplace_separate_view') && isset($this->session->data['marketplace_separate_view']) && $this->session->data['marketplace_separate_view'] == 'separate' && preg_match('/common\/filemanager/',$route) && isset(explode('/',$route,2)[1])) {
                $output = $template->render('default/template/common/filemanager', $this->registry->get('config')->get('template_cache'));
              } else {
