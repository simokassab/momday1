...
class ControllerCustomerCustomer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['create_seller'])) {
                $this->model_customer_customer->addCustomer($this->request->post, $create_seller = 1);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['customer_id'])) {
            ...
            if (isset($this->request->get['create_seller'])) {
                ...
                $data['action'] = $this->url->link('customer/customer/add', 'user_token=' . $this->session->data['user_token'] . $url. '&create_seller', true);
            }else{  // Marketplace Code Ends Here
