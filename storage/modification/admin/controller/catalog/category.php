		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['page'])) {
              ...
              if ($this->config->get('module_marketplace_status') && isset($this->request->get['mpcheck'])) {
                  $this->response->redirect($this->url->link('customerpartner/sellercategory', 'user_token=' . $this->session->data['user_token'] . $url, true));
              } else {
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('catalog/category/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
              ...
              if (isset($this->request->get['mpcheck']) && $this->request->get['mpcheck']) {
                $data['action'] = $this->url->link('catalog/category/edit', 'user_token=' . $this->session->data['user_token'] . '&mpcheck=1&category_id=' . $this->request->get['category_id'] . $url, true);
              } else {
