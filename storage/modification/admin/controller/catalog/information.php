	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
          ...
          if ($this->config->get('module_marketplace_status') && isset($this->request->get['mpcheck'])) {
              $this->response->redirect($this->url->link('customerpartner/information', 'user_token=' . $this->session->data['user_token'] . $url, true));
          } else {
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['information_id'])) {
          ...
          if (isset($this->request->get['mpcheck']) && $this->request->get['mpcheck']) {
            $data['action'] = $this->url->link('catalog/information/edit', 'user_token=' . $this->session->data['user_token'] . '&mpcheck=1&information_id=' . $this->request->get['information_id'] . $url, true);
          } else {
